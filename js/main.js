...
(function initializeOperationsInterface() {
    ...
    document.querySelectorAll('.project-card').forEach((card, index) => {
        card.style.setProperty('--project-index', String(index + 1).padStart(2, '0'));
    });
})();


(function initializeDataIntelligenceLab() {
    const line = document.getElementById('throughput-line');
    const area = document.getElementById('throughput-area');
    const pointsGroup = document.getElementById('throughput-points');
    const throughputValue = document.getElementById('live-throughput');
    if (!line || !area || !pointsGroup) return;

    const values = [42, 51, 46, 63, 58, 71, 67, 82, 76, 91, 87, 103, 96, 112, 101, 118, 109, 116, 104, 111, 99, 108, 95, 102];
    const max = 125;
    const width = 800;
    const top = 20;
    const bottom = 239;
    ...
    points.filter((_, index) => index % 4 === 0 || index === points.length - 1).forEach(([x, y]) => {
        const circle = document.createElementNS('http://www.w3.org/2000/svg', 'circle');
        circle.setAttribute('cx', x);
        circle.setAttribute('cy', y);
        circle.setAttribute('r', '4');
        circle.setAttribute('class', 'throughput-point');
    ...
    if (throughputValue) {
        let current = 86420;
        window.setInterval(() => {
            current = Math.max(78000, Math.min(94000, current + Math.round((Math.random() - .48) * 1800)));
            throughputValue.textContent = current.toLocaleString('en-US');
        }, 1800);
    }
})();


(function initializeDataLabReveal() {
    const lab = document.getElementById('data-lab');
    if (!lab) return;

    const reveal = () => lab.classList.add('lab-visible');
...
(function initializePlatformV2(){
  const boot=document.getElementById('platformBoot');
  const reduce=window.matchMedia('(prefers-reduced-motion: reduce)').matches;
  if(boot){window.setTimeout(()=>boot.classList.add('boot-complete'),reduce?0:2100);window.setTimeout(()=>boot.remove(),reduce?50:3000)}
  const cards=[...document.querySelectorAll('#projects .project-card')];
  const metrics=[
    ['ONLINE','84 ms','12.8M'],['ONLINE','1.2 s','8.4M'],['ONLINE','96 ms','5.7M'],['ONLINE','340 ms','3.1M'],
    ['ONLINE','210 ms','9.6M'],['ONLINE','440 ms','1.8M'],['ONLINE','128 ms','6.2M'],['ONLINE','72 ms','14.5M']
  ];
  cards.forEach((card,index)=>{
    if(card.querySelector('.project-system-metrics'))return;
    const m=metrics[index%metrics.length];
    const block=document.createElement('div');block.className='project-system-metrics';
    block.innerHTML=`<div><small>STATUS</small><strong class="online">${m[0]}</strong></div><div><small>LATENCY</small><strong>${m[1]}</strong></div><div><small>RECORDS</small><strong>${m[2]}</strong></div>`;
    const tech=card.querySelector('.project-tech'); if(tech) tech.before(block); else card.querySelector('.project-details')?.append(block);
  });
  const toast=document.getElementById('systemToast');
  const sections=[
    [document.querySelector('.hero'),'ENGINEER PROFILE AUTHENTICATED'],
    [document.getElementById('about'),'PROFILE MODULE ONLINE'],
    [document.getElementById('data-lab'),'DATA INTELLIGENCE LAB ONLINE'],
    [document.getElementById('projects'),'PROJECT SYSTEMS ONLINE'],
    [document.getElementById('contact'),'COMMUNICATION CHANNEL OPEN']
  ].filter(([el])=>el);
  sections.forEach(([el,label])=>el.dataset.bootLabel=label);
  if('IntersectionObserver' in window){
    const io=new IntersectionObserver(entries=>entries.forEach(entry=>{if(!entry.isIntersecting)return;entry.target.classList.add('system-online');if(toast){toast.textContent='// '+entry.target.dataset.bootLabel;toast.classList.add('show');clearTimeout(toast._t);toast._t=setTimeout(()=>toast.classList.remove('show'),1300)}io.unobserve(entry.target)}),{threshold:.25});
    sections.forEach(([el])=>io.observe(el));
  }
  const kafka=document.getElementById('kafkaRate'),spark=document.getElementById('sparkJobs'),air=document.getElementById('airflowDag'),wh=document.getElementById('warehouseLoad');
  setInterval(()=>{
    if(kafka)kafka.textContent=(82+Math.random()*6).toFixed(1)+'K/s';
    if(spark)spark.textContent=(10+Math.floor(Math.random()*5))+' RUNNING';
    if(air)air.textContent=(27+Math.floor(Math.random()*2))+'/28 HEALTHY';
    if(wh)wh.textContent=(58+Math.floor(Math.random()*13))+'% LOAD';
  },2200);
})();
